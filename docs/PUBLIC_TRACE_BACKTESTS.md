...
| **ShareGPT (RyokoAI/ShareGPT52K)** | EVAL-CLASS conversation-shape PROXY for the Eval Workload Frontier v1 (sibling of the serving / training frontiers). Carries turn count + role sequence + char-derived token-estimate **proxy**; no arrival timestamps, no measured latency, no model id, no real tokenizer. | **Implemented as bounded-ingest only** (`aurelius/traces/sharegpt_aiperf.py`, `scripts/ingest_sharegpt_aiperf.py`). HTTP-Range bounded head download (default 50 MB) from `https://huggingface.co/datasets/RyokoAI/ShareGPT52K/resolve/main/old/sg_52k.json`. Processed summary committed under `data/external/sharegpt_aiperf/processed/`. **NOT customer telemetry. NOT a measured serving trace.** See `docs/EVAL_AND_BATCH_FRONTIER_RESULTS.md`. |
| **LMSYS Chatbot Arena conversations** | EVAL-CLASS conversation-shape (real `tstamp` + `model_a/b` + multi-turn) source for the Eval Workload Frontier v1. **Gated** on HuggingFace; auto-download requires accepting LMSYS terms-of-use + supplying `HF_TOKEN`. | **Schema + adapter committed, bounded ingest gated** (`aurelius/traces/lmsys_chatbot_arena.py`, `scripts/ingest_lmsys_chatbot_arena.py`). The schema is transcribed from the dataset card so tests can pin it without the dataset; `normalize_record` maps an LMSYS row + side onto `EvalWorkloadRequest`. `download_gated()` REFUSES to proceed without a token — the gated path is never silently bypassed. See `docs/EVAL_AND_BATCH_FRONTIER_RESULTS.md`. |
| **Azure Functions 2019** | Short-task arrival-shape PROXY for ETL / embedding fan-out workloads. Multi-file, ~25 GB raw. | **DEFERRED_BOUNDED_INGEST** — too large + complex for the v1 batch / eval frontier; revisit when an ETL frontier is built. URL: https://github.com/Azure/AzurePublicDataset/blob/master/AzureFunctionsDataset2019.md. |
